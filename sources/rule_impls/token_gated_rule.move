...
public struct TokenGatedRule has drop {}
...
public fun add<OP, T>(set: &mut RuleSet<OP>, cap: &RuleSetCap, min_balance: u64, required: bool) {
    rules::add(TokenGatedRule {}, set, cap, Config<T> { min_balance }, required)
}

public fun remove<OP, T>(set: &mut RuleSet<OP>, cap: &RuleSetCap) {
    let Config<T> { min_balance: _ } = rules::remove<OP, TokenGatedRule, Config<T>>(set, cap);
}
...
public fun prove<OP, T>(set: &RuleSet<OP>, req: &mut Request<OP>, proof: &Coin<T>) {
    let config = rules::config<OP, TokenGatedRule, Config<T>>(set);
    assert!(proof.value() >= config.min_balance, EInsufficientBalance);
    rules::add_approval(TokenGatedRule {}, set, req);
}

public fun min_balance<OP, T>(set: &RuleSet<OP>): u64 {
    rules::config<OP, TokenGatedRule, Config<T>>(set).min_balance
}
